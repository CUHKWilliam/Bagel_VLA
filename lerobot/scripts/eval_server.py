...
def server_start(cfg: TrainPipelineConfig):
    ...
    checkpoint_path = cfg.output_dir / "checkpoints" / "pytorch_model.bin"
    ckpt = torch.load(open(checkpoint_path, 'rb'))
    ...
    print(f"vllm service ready")
    while True:
        listen_and_process_input(unwrapped_policy)
