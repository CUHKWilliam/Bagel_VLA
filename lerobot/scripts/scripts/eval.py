...
def rollout(
    ...
    raw_observation = {
        "pixels":{
            "agentview_image": observation1[::-1, ::-1, :].copy(),
            "robot0_eye_in_hand_image": observation2[::-1, ::-1, :].copy(),
        }
    }
    ...
    while not done:
        ...
        raw_observation['pixels'] = {
                "agentview_image": new_observation['agentview_image'][::-1, ::-1, :].copy(),
                "robot0_eye_in_hand_image": new_observation['robot0_eye_in_hand_image'][::-1, ::-1, :].copy(),
        }
...
def eval_policy(
    ...
    def render_frame(env, env_id):
        ...
        if n_episodes_rendered >= max_episodes_rendered:
            return
        image = env.step([0] * 7)[0]['agentview_image'][::-1, ::-1, :].copy()
