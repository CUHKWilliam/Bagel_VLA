...
from lerobot.common.policies.pi0.modeling.qwen2 import (
    Qwen2Attention, 
    Qwen2MLP, 
...
from lerobot.common.policies.pi0.modeling.qwen2 import Qwen2Config as _Qwen2Config
...
class PackedAttentionMoT2(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        if self.config.qk_norm:
            self.q_norm = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            ...
            self.q_norm_moe_gen = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.k_norm_moe_gen = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.q_norm_moe_gen2 = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.k_norm_moe_gen2 = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps)
        else:
            self.q_norm = nn.Identity()
            self.k_norm = nn.Identity()
            self.q_norm_moe_gen = nn.Identity()
        ...
        self.q_proj_moe_gen2 = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=True)
        self.k_proj_moe_gen2 = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj_moe_gen2 = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj_moe_gen2 = nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=False)
    ...
    def forward_train(
        ...
        if packed_gen_token_indexes is not None  and len(packed_gen_token_indexes) > 0:
            packed_sequence_gen = packed_sequence[packed_gen_token_indexes]
        packed_sequence_act = packed_sequence[packed_action_token_indexes]
        packed_query_states[packed_und_token_indexes] = self.q_proj(packed_sequence_und)
        if packed_gen_token_indexes is not None  and len(packed_gen_token_indexes) > 0:
            packed_query_states[packed_gen_token_indexes] = self.q_proj_moe_gen(packed_sequence_gen)
        packed_query_states[packed_action_token_indexes] = self.q_proj_moe_gen2(packed_sequence_act)

        packed_key_states[packed_und_token_indexes] = self.k_proj(packed_sequence_und)
        if packed_gen_token_indexes is not None and len(packed_gen_token_indexes) > 0 :
            packed_key_states[packed_gen_token_indexes] = self.k_proj_moe_gen(packed_sequence_gen)
        packed_key_states[packed_action_token_indexes] = self.k_proj_moe_gen2(packed_sequence_act)

        packed_value_states[packed_und_token_indexes] = self.v_proj(packed_sequence_und)
        if packed_gen_token_indexes is not None  and len(packed_gen_token_indexes) > 0:
            packed_value_states[packed_gen_token_indexes] = self.v_proj_moe_gen(packed_sequence_gen)
        packed_value_states[packed_action_token_indexes] = self.v_proj_moe_gen2(packed_sequence_act)
        ...
        if packed_gen_token_indexes is not None and len(packed_gen_token_indexes) > 0:
            packed_query_states_[packed_gen_token_indexes] = self.q_norm_moe_gen(packed_query_states[packed_gen_token_indexes])
        packed_query_states_[packed_action_token_indexes] = self.q_norm_moe_gen2(packed_query_states[packed_action_token_indexes])
        ...
        if packed_gen_token_indexes is not None  and len(packed_gen_token_indexes) > 0:
            packed_key_states_[packed_gen_token_indexes] = self.k_norm_moe_gen(packed_key_states[packed_gen_token_indexes])
        packed_key_states_[packed_action_token_indexes] = self.k_norm_moe_gen2(packed_key_states[packed_action_token_indexes])
        ...
        if packed_gen_token_indexes is not None and len(packed_gen_token_indexes) > 0:
            packed_attn_output_[packed_gen_token_indexes] = self.o_proj_moe_gen(packed_attn_output[packed_gen_token_indexes])
        packed_attn_output_[packed_action_token_indexes] = self.o_proj_moe_gen2(packed_attn_output[packed_action_token_indexes])
    ...
    def forward_inference(
        ...
        if mode == 'und':
            ...
            packed_query_states[packed_text_indexes] = self.q_proj(packed_text_query_sequence)
            packed_query_states[packed_action_token_indexes] = self.q_proj_moe_gen2(packed_action_query_sequence)

            packed_key_states[packed_text_indexes] = self.k_proj(packed_text_query_sequence)
            packed_key_states[packed_action_token_indexes] = self.k_proj_moe_gen2(packed_action_query_sequence)

            packed_value_states[packed_text_indexes] = self.v_proj(packed_text_query_sequence)
            packed_value_states[packed_action_token_indexes] = self.v_proj_moe_gen2(packed_action_query_sequence)
            ...
            packed_query_states = packed_query_states.to(torch.float32)
            packed_query_states[packed_text_indexes] = self.q_norm(packed_query_states[packed_text_indexes])
            packed_query_states[packed_action_token_indexes] = self.q_norm_moe_gen2(packed_query_states[packed_action_token_indexes])
        ...
        if mode == 'und':
            ...
            packed_attn_output[packed_text_indexes] = self.o_proj(packed_attn_output[packed_text_indexes])
            packed_attn_output[packed_action_token_indexes] = self.o_proj_moe_gen2(packed_attn_output[packed_action_token_indexes])
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = attn_module(config, layer_idx, visual_gen=visual_gen)
    ...
    def forward_inference(
        ...
        if mode == "action":
            packed_query_sequence[packed_action_token_indexes] *= 0.
        packed_query_sequence = residual + packed_query_sequence
...
class Qwen2MoTDecoderLayer2(nn.Module):
    def __init__(
        self, 
        config, 
        layer_idx: Optional[int] = None, 
        attn_module: Optional[Qwen2Attention] = PackedAttentionMoT2,
    ):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.freeze_und = config.freeze_und
        ...
        self.mlp = Qwen2MLP(config)
        self.mlp_moe_gen = Qwen2MLP(config)
        self.mlp_moe_gen2 = Qwen2MLP(config)
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm_moe_gen2 = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm_moe_gen2 = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward_train(
        ...
        packed_und_token_indexes: torch.LongTensor,
        packed_gen_token_indexes: torch.LongTensor | None,
        packed_action_token_indexes: torch.LongTensor | None,
    ) -> torch.Tensor:
    ...
    def forward_inference(
        ...
        if mode == "und":
            ...
            packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
            packed_query_sequence_[packed_text_indexes] = self.input_layernorm(packed_query_sequence[packed_text_indexes])
            packed_query_sequence_[packed_action_token_indexes] = self.input_layernorm_moe_gen2(packed_query_sequence[packed_action_token_indexes])
            packed_query_sequence = packed_query_sequence_
        ...
        if mode == "und":
            ...
            packed_action_query_sequence = packed_query_sequence[packed_action_token_indexes]
            packed_text_query_sequence = self.post_attention_layernorm(packed_text_query_sequence)
            packed_action_query_sequence = self.post_attention_layernorm_moe_gen2(packed_action_query_sequence)
            packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
            packed_query_sequence_[packed_text_indexes] = self.mlp(packed_text_query_sequence)
            packed_query_sequence_[packed_action_token_indexes] = self.mlp_moe_gen2(packed_action_query_sequence)
            packed_query_sequence = packed_query_sequence_
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config, visual_gen=True):
        ...
        self.layers = nn.ModuleList(
            [layer_module(config, layer_idx, visual_gen=visual_gen) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def init_moe(self):
        for name, param in self.named_parameters():
            if "moe_gen" in name and "moe_gen2" not in name:
                original_name = name.replace("_moe_gen", "")
                if original_name in self.state_dict().keys():
                    param.data.copy_(self.state_dict()[original_name].data)
            elif "moe_gen2" in name:
                original_name = name.replace("_moe_gen2", "")
                if original_name in self.state_dict().keys():
                    param.data.copy_(self.state_dict()[original_name].data)
