...
class PI0Policy(PreTrainedPolicy):
    ...
    def generate_image(self, batch ):
        self.eval()
        predict_image = self.model.sample_images(
    ...
    def prepare_inputs(self, data_batch):
        ...
        data_batch = SimpleCustomBatch([data_batch]).cuda(f"cuda:{torch.cuda.current_device()}").to_dict()
...
class PI0FlowMatching(nn.Module):
    ...
    def sample_images(self, batch, vit_transform, vae_transform, tokenizer):
        ...
        return predict_images[0], Image.fromarray(observation_image)
