...
class PackedDataset:
    ...
    def __call__(self, batch_dataloader, tokenize_action):
        ...
        while True:
            # IMPORTANT:
            # - Keep a persistent iterator over the dataloader.
            # - Handle StopIteration by restarting the iterator.
            # This avoids repeatedly restarting the dataloader (which can desync ranks and/or stall).
            try:
                dl_iter
            except NameError:
            ...
            while True:
                ...
                # Avoid full deepcopy (very expensive); pack_sequence only mutates list-like fields.
                sample_test = copy.deepcopy(sample)
                if 'image_tensor_list' in sample:
                    sample_test['image_tensor_list'] = list(sample['image_tensor_list'])
                if 'text_ids_list' in sample:
                    sample_test['text_ids_list'] = list(sample['text_ids_list'])
                if 'action' in sample:
                    sample_test['action'] = list(sample['action'])
