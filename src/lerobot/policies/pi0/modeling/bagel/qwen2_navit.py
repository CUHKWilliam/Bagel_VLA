...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if past_key_values is not None and past_key_values.key_cache[self.layer_idx] is not None:
            ...
            past_key_unnorm_states = past_key_values.key_unnorm_cache[self.layer_idx]
            merged_key_unnorm_states = past_key_unnorm_states.new_zeros(size=[seqlens, self.num_key_value_heads, self.head_dim])
            merged_key_unnorm_states[packed_query_indexes] = packed_key_unnorm_states
            merged_key_unnorm_states[packed_key_value_indexes] = past_key_unnorm_states
        else:
            merged_key_states = packed_key_states
            merged_value_states = packed_value_states
            key_values_lens = query_lens
            merged_key_unnorm_states = packed_key_unnorm_states
