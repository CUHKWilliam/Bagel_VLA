...
--dataset.repo_id="/dataset_rc_mm/share/datasets/modelscope.cn/Galaxea/Galaxea-Open-World-Dataset/lerobot_decompressed/*" \
--policy.model_size=$MODEL_SIZE \
--dataset.token_num=$TOKEN_NUM \
--output_dir="./outputs/train/formal_train_galaxea" \
--policy.push_to_hub=false \
--policy.repo_id="lerobot/pi0" \
--eval_freq=20000000 \
--save_freq=1000 \
--job_name="formal_train_galaxea" \
--wandb.project="exp_formal" \
--policy.use_ref false \
--policy.type="pi0" \
--resume true \
--config_path="./outputs/train/formal_train/checkpoints/last/pretrained_model/train_config.json" \
--resume true \
